...
const caseBlocks = (image: SchemaContext['image']) => {
    ...
    items: z
      .array(
        z.object({
           ...
           * subtle — светло-зелёная, quote — янтарная с цитатой респондента:
           * у неё в макете и палитра своя, и иконка.
           */
          tone: z.enum(['solid', 'subtle', 'quote']).default('subtle'),
        }),
      )
      .min(1),
  });
  ...
  const headingBlock = z.object({
    ...
    /**
     * Кегль подзаголовка. h4 (24/32) — как в первом кейсе и в первом шаге
     * второго; h5 (20/26) — как в шагах 2 и 3 второго кейса, где макет
     * набирает шаг тем же кеглем, что и заголовки блоков. Отступ сверху
     * компонент подбирает под кегль сам, чтобы содержимое в обоих случаях
     * начиналось на 42px от начала раздела.
     */
    size: z.enum(['h4', 'h5']).default('h4'),
  });
