...
const caseBlocks = (image: SchemaContext['image']) => {
  ...
  const sliderBlock = z.object({
    ...
    /**
     * Стрелки и точки. auto — появляются, когда листать есть что; always —
     * стоят всегда, даже при одном слайде. Второе нужно там, где экраны ещё
     * будут добавляться: в макете второго кейса управление нарисовано у всех
     * слайдеров, и место под него не должно поехать, когда слайды приедут.
     * В первом кейсе макет у одиночного слайдера управление не рисует.
     */
    controls: z.enum(['auto', 'always']).default('auto'),
