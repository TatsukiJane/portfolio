...
const caseBlocks = (image: SchemaContext['image']) => {
  ...
  const metricsBlock = z.object({
    ...
    /**
     * Строка над плитками — например, откуда взяты числа. В макете она
     * отбита от плиток 24px, то есть входит в тот же блок, а не стоит
     * отдельным абзацем.
     */
  ...
  const sliderBlock = z.object({
    ...
    /**
     * Какие мокапы стоят в слайдере. phone — ряд экранов 288×624 в рамках
     * телефонов, подписи идут рядом под ними; desktop — один экран 1008×630
     * в рамке браузера, а подписи лежат поверх него по координатам макета
     * (см. `callouts`).
     */
    ...
    slides: z
      .array(
        z.object({
          ...
          /**
           * Подписи поверх экрана — так набраны комментарии в веб-кейсе, где
           * экран один и места под ним нет. Координаты от левого верхнего
           * угла ряда экранов, как у выделений и выносок; высоту плашка
           * берёт по тексту.
           */
          callouts: z
            .array(
              z.object({
                ...
                y: z.number(),
                width: z.number(),
                /** В макете одна подпись набрана по центру, остальные слева. */
...
const caseSection = (image: SchemaContext['image']) =>
  z.object({
    /**
     * Заголовок в левой колонке. У раздела-продолжения его нет: во втором
     * кейсе шаги «Решений» отбиты разделителями, как отдельные разделы,
     * но заголовок у второго шага в макете скрыт. Шаги такого раздела
     * оглавление дописывает в главу, которая идёт выше.
     */
    title: z.string().optional(),
...
const cases = defineCollection({
  ...
  schema: ({ image }) =>
    z.object({
      ...
      /**
       * Экран в первом экране страницы кейса. Нужен кейсам про веб: там
       * в макете один десктопный экран вместо ряда телефонов — контейнер
       * 1240×760, сам экран 1104×632. Без него первый экран собирается
       * из `shots`, как у мобильного кейса.
       */
      cover: z
        .object({
          src: image().optional(),
